...
function addboxes() {
    ...
    if (x == "" ) {
        ...
        childbox.innerHTML = `${x} <hr> <i style="display: block; color: rgb(101, 1, 215); cursor: pointer;position: absolute; top: 262px; right: 20px;" id="cardadd" onclick="cardadd(this)" class="fa-solid fa-circle-plus"></i>
        <i style="display: block; color: red; position: absolute; top: 262px;cursor: pointer; right: 61px;" id="carddelete" onclick="carddelete(this)" class="fa-solid fa-trash"></i>`
...
function inrtext() {
    ...
    inrTxt.innerHTML = `<span style = "text-align:left"  class = "inserted" >${input2}</span>
     <span id="read" style="width: 45px; border: 2px solid rgb(102, 0, 255); font-size: 15px;
     background-color:rgb(102, 0, 255);border-radius:10px ;position:relative; top:7px; color: #fff;
     font-weight: 600; height: 15px;line-height:13px; cursor:pointer; float:right;text-decoration: yellow" 
     class= "read" >Done</span>`;
